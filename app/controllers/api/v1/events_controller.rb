...
class Api::V1::EventsController < EventsController
  ...
  def responses
    begin
      ...
      @suggested_activities = Notification.where(event: @event, has_suggested_activity: true)
      @suggested_locations = Notification.where(event: @event, has_suggested_location: true)
      @suggested_datetimes = Notification.where(event: @event, has_suggested_datetime: true)
    rescue ActiveRecord::RecordNotFound
      render nothing: true, status: :not_found
    end
  end
